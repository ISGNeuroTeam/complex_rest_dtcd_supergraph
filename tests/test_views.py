...
URL_RESET = reverse("dtcd_supergraph:reset")  # post here resets the db
...
class TestFragmentListView(APISimpleTestCase):
    def setUp(self) -> None:
        ...
        self.url = reverse("dtcd_supergraph:fragments")
...
class TestFragmentDetailView(APISimpleTestCase):
    def setUp(self) -> None:
        ...
        response = self.client.post(
            reverse("dtcd_supergraph:fragments"),
            data={"name": "sales"},
            format="json",
        )
        ...
        self.url = reverse(
            "dtcd_supergraph:fragment-detail", args=(self.pk,)
        )
...
class TestNeo4jGraphView(APISimpleTestCase):
    @classmethod
    def setUpClass(cls):
        cls.url_fragments = reverse("dtcd_supergraph:fragments")
        cls.url_reset = reverse("dtcd_supergraph:reset")
    ...
    def setUp(self):
        ...
        self.url_graph = reverse(
            "dtcd_supergraph:fragment-graph", args=(self.fragment_id,)
        )
